...
public class SingaporeCurator extends Curator {
    ...
    @Inject
    protected SingaporeCurator(SearchClient searchClient) {
        super(searchClient);
    }
     ...
     */
    protected static SearchCollection locationQuery(String name, SearchQuery source, String[] points) {
        SearchQuery query = clone(source);
        query.setLocation(new Location());
        query.getLocation().setPoints(Arrays.asList(points));
        return new SearchCollection(name, query);
    }
}
