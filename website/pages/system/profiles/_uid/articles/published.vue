  ...
  <div class="container pt-48 pb-64">
    ...
    <div>
      <div class="hr-bot mt-32 text-decoration-none flex-row">
        <nuxt-link :to="`/system/profiles/${profile.uid}/articles/drafts`" class="mr-24 b-a40 ptb-8">
          Drafts
        </nuxt-link>
        <nuxt-link :to="`/system/profiles/${profile.uid}/articles/published`" class="ptb-8 black hr-bot Selected">
          Published
        </nuxt-link>
      </div>
    </div>
    ...
    <div>
      ...
      <div v-else>
        <div class="ptb-48 flex-center">
          <p>{{profile.name}} don't have any published content yet.</p>
        </div>
      </div>
    </div>
  ...
  export default {
    asyncData({$api, params: {uid}}) {
      return $api.get(`/admin/profiles/${uid}`)
        .then(({data: profile}) => {
          return $api.get(`/admin/profiles/${uid}/articles`, {params: {status: 'PUBLISHED', size: 20}})
