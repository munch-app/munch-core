  ...
  <div class="container pt-64 pb-64">
    ...
    <div>
      <div class="hr-bot mt-32 text-decoration-none flex-row">
        <nuxt-link to="/me/articles/drafts" class="mr-24 b-a40 ptb-8">
          Drafts
        </nuxt-link>
        <nuxt-link to="/me/articles/published" class="ptb-8 black hr-bot Selected">
          Published
        </nuxt-link>
      </div>
    </div>
    ...
    <div>
      ...
      <div v-else>
        <div class="ptb-48 flex-center">
          <p>You don't have any published content yet.</p>
        </div>
      </div>
    </div>
...
<script>
  ...
  export default {
    head() {
      return {
        title: `Published - Articles · ${this.name}`
      }
    },
    computed: {
    ...
    asyncData({$api}) {
      return $api.get('/me/articles', {params: {status: 'PUBLISHED', size: 20}})
    ...
    methods: {
      ...
      onLoadMore() {
        this.$api.get('/me/articles', {params: {status: 'PUBLISHED', size: 20, cursor: this.next}})
          .then(({data: articles, cursor}) => {
