...
public class ImageMapper {
     ...
     *
     * @see ImageMapper#upload(String, File, ImageKind...)
     */
    public Image upload(String fileName, File file, Set<ImageKind> kinds) throws ContentTypeError, IOException {
        return upload(fileName, file, kinds.toArray(new ImageKind[kinds.size()]));
    }
     ...
     */
    public Image upload(String fileName, File file, ImageKind... kinds) throws ContentTypeError, IOException {
        ...
        original.setKey(randomImageKey(fileName, file));
        image.getKinds().add(original);

        // Persist Original Size
        fileMapper.put(original.getKey(), file);
        database.put(image);
    ...
    public void resolveUrl(Image image) {
        for (Image.Kind kind : image.getKinds()) {
            kind.setUrl(fileMapper.getUrl(kind.getKey()));
        }
    }
    ...
    private String randomImageKey(String fileName, File file) throws ContentTypeError {
        try {
            MimeType mimeType = tika.getMimeRepository().forName(FileTypeUtils.getContentType(fileName, file));
            return RandomStringUtils.randomAlphanumeric(40) + "." + mimeType.getExtension();
        } catch (MimeTypeException e) {
            throw new RuntimeException(e);
        }
    }

    private String randomImageKey(String fileName) throws ContentTypeError {
        try {
